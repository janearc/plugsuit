...
function get_tasks( dir ) {
	return dg.flatten( rds( dir ).filter( function (f) {
		return is_file( f ) ? f : is_dir( f ) ? rds( f ) : [ ]
	} ).map( function (f) {
		// Attempt to return a fully-qualified path to the file or return nothing
		//
		var p = require( 'path' );
		if (is_file( p.resolve( f, dir ))) {
			return p.resolve( f, dir );
		}
		else {
			return [ ];
		}
...
module.exports = { init: plugsuit.init }
